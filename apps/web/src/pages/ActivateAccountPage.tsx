...
export default function ActivateAccountPage() {
  ...
  return <main className="center-screen"><div className="card activation-card"><img src="/logo-icon.svg" width="48" height="48" alt="" /><h1>Activez votre accès</h1>{loading ? <p>Vérification de l’invitation…</p> : invitation ? <><p><strong>{invitation.name}</strong>, vous rejoignez {invitation.organizationName}.</p>{error && <div className="error-banner">{error}</div>}<form onSubmit={submit}><div className="field"><label>Votre mot de passe</label><div className="password-input"><input type={showPassword ? "text" : "password"} value={password} onChange={(event) => setPassword(event.target.value)} minLength={8} autoComplete="new-password" required /><button type="button" className="password-toggle" onClick={() => setShowPassword((v) => !v)} aria-label={showPassword ? "Masquer le mot de passe" : "Afficher le mot de passe"} aria-pressed={showPassword}><Icon name={showPassword ? "eye-off" : "eye"} /></button></div></div><div className="field"><label>Confirmer le mot de passe</label><div className="password-input"><input type={showPassword ? "text" : "password"} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} minLength={8} autoComplete="new-password" required /><button type="button" className="password-toggle" onClick={() => setShowPassword((v) => !v)} aria-label={showPassword ? "Masquer le mot de passe" : "Afficher le mot de passe"} aria-pressed={showPassword}><Icon name={showPassword ? "eye-off" : "eye"} /></button></div></div><button className="btn block">Activer mon compte</button></form></> : <div className="error-banner">{error ?? "Invitation invalide ou expirée"}</div>}</div></main>;
}
