...
function UserDetail({ user, isSelf, isLastActiveAdmin, onClose, onUpdate, onDelete }: { user: UserDTO; isSelf: boolean; isLastActiveAdmin: boolean; onClose: () => void; onUpdate: (input: { name?: string; phone?: string | null; employerCompany?: string | null; role?: UserRole; isActive?: boolean }) => Promise<void>; onDelete: () => Promise<void> }) {
  ...
  return <div className="sheet-overlay" onClick={onClose}><aside className="sheet user-detail" onClick={(event) => event.stopPropagation()} aria-labelledby="user-detail-title">
    ...
    {isSelf && <div className="user-self-notice"><strong>Votre compte administrateur</strong><span>Pour éviter de perdre l’accès à l’entreprise, vous ne pouvez pas modifier votre rôle, désactiver ou supprimer ce compte.</span></div>}
